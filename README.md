...
AI 可调用全部内置工具（19 个，含「搜索 → 按 pick_code 直下」链路）；需要新能力时它会写一个受限沙箱内的 Python 小工具，**经你在 TG 点确认后**才启用（`/aitools` 管理）。会话记忆持久化，重启不丢；`/ai off` 临时停用。

### Web 管理台（可选）

不用 SSH 敲命令，浏览器里管好一切：看传输进度、整理 115 网盘、改配置、续授权，远程服务器上尤其顺手。`web.enable: true` 后访问 `http://<host>:<port>/`（HTTP Basic 认证，凭据见 `config.web`）。

- **仪表盘** — 实时传输进度（3s 刷新）、115 空间 / API 余量、磁盘水位、账号状态、一键服务诊断
- **文件** — 115 网盘管理：目录浏览、全盘搜索、删除 / 重命名 / 移动 / 新建目录，服务器端下载与本地路径上传（进度实时可见）
- **离线任务** — 磁力 / ed2k / 直链提交下载，列表自动刷新
- **任务历史** — 全部传输记录，终态任务可删记录
- **账号** — 多账号轮转状态与故障冷却
- **频道规则** — 频道监控规则在线增删
- **日志** — 业务日志（级别筛选）+ 运行日志（stdout 实时跟踪）
- **配置** — config.yaml 在线编辑（校验 + 自动备份 + 快捷开关）与 115 扫码授权
